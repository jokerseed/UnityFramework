...
namespace KcpCSharp {
    ...
    */
    public class Kcp {
        ...
        /// <summary>可由 SetMaxRto 配置；默认等于 IKCP_RTO_MAX。实时对战建议 capped 在 500～3000ms。</summary>
        private int rx_rto_max = IKCP_RTO_MAX;
        private int snd_wnd;
        private int rcv_wnd;
        private int rmt_wnd;
        private int cwnd;
        ...
        private int nocwnd;
        private bool stream;
        /// <summary>为 true 时，Input 内若仅有待发 ACK（窗口未滑动）则立即 flush(true)，与 kcp-go Input 的 ackNoDelay 一致。</summary>
        private bool ackNoDelay = true;
        private int logmask;
        ...
        private void Update_ack(int rtt) {
            ...
            rx_rto = _ibound_(rx_minrto, rto, rx_rto_max);
        }
        ...
        public int Input(ByteBuf data) {
            ...
            if (_itimediff(snd_una, una_temp) > 0) {
                ...
                // 与 kcp-go 一致：发送窗口随 una 推进后立即 flush
                Flush(false);
            } else if (ackNoDelay && acklist.Count > 0) {
                // 与 kcp-go 一致：未滑动发送窗口但已有 ACK 待发出时立即只发 ACK（flush true）
                Flush(true);
            }
            return 0;
        }
         ...
         * flush pending data
         */
        private void Flush(bool ackOnly) {
            int cur = current;
            int change = 0;
            int lost = 0;
            if (updated == 0) {
            ...
            for (int i = 0; i < snd_queue.Count; i++) {
                ...
                newseg.una = rcv_nxt;
                newseg.resendts = cur;
                newseg.rto = _ibound_(rx_minrto, rx_rto, rx_rto_max);
                newseg.fastack = 0;
                newseg.xmit = 0;
                snd_buf.Add(newseg);
                c++;
            ...
            for (int i = 0; i < snd_buf.Count; i++) {
                ...
                if (segment.xmit == 0) {
                    needsend = true;
                    segment.xmit++;
                    segment.rto = _ibound_(rx_minrto, rx_rto, rx_rto_max);
                    ...
                    segment.rto = _ibound_(rx_minrto, segment.rto, rx_rto_max);
        ...
        public void SetMinRto(int min) {
            rx_minrto = min;
            if (rx_rto_max < rx_minrto)
                rx_rto_max = rx_minrto;
        }

        /// <summary>
        /// 限制根据 RTT 计算出的 rx_rto 及单段重传 RTO 上限（毫秒）。
        /// 默认 60000；弱网实时对战建议 500～3000，避免单次抖动后重传退避拖到数秒。
        /// </summary>
        public void SetMaxRto(int maxMs) {
            if (maxMs < rx_minrto)
                maxMs = rx_minrto;
            if (maxMs > IKCP_RTO_MAX)
                maxMs = IKCP_RTO_MAX;
            rx_rto_max = maxMs;
            if (rx_rto > rx_rto_max)
                rx_rto = rx_rto_max;
        }
        ...
        /// <summary>
        /// 在本轮 Input/Flush 之前写入当前毫秒时钟（与 KcpOnUdp 的 Environment.TickCount 一致），
        /// 避免 Input 里 ACK 的 RTT 仍用上一轮 Update 留下的 current。
        /// </summary>
        public void SyncMillis(int millis) {
            this.current = millis;
        }

        /// <summary>与 kcp-go UDPSession.SetACKNoDelay 对齐；false 时不在 Input 内做仅 ACK 的立即 flush。</summary>
        public void SetACKNoDelay(bool enable) {
            this.ackNoDelay = enable;
        }

        /// <summary>
        /// 刚完成 Input 或上层 Send 后，若仅依赖 Update 的 ts_flush 节拍，可能出现 slap&lt;0 而本轮不 Flush，
        /// ACK/新数据会拖到下一个 interval（弱网下与 RTT 叠加易被感知成秒级）。将 ts_flush 拉回当前时刻，保证紧随其后的 Update 会 Flush。
        /// </summary>
        public void MarkFlushImmediate(long currentMillis) {
            int c = (int)currentMillis;
            this.current = c;
            if (updated == 0) {
                updated = 1;
                ts_flush = c;
