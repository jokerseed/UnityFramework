...
namespace Framework.Lockstep.Physics3D
{
    ...
    public class CollisionSystemPersistentSAP : CollisionSystem
    {
        ...
        ~CollisionSystemPersistentSAP()
        {
            ...
            UnityEngine.Debug.Log("CollisionSystemPersistentSAP");
                ...
                while (i >= 0 && axis[i].Value > key)
                {
                    ...
                    if (!keyelement.Begin && swapper.Begin)
                    {
                        lock (fullOverlaps)
                        {
                            ...
                            RigidBody rigidBody1 = swapper.Body as RigidBody;
                            RigidBody rigidBody2 = keyelement.Body as RigidBody;
                            GameObject go1 = null;
                            GameObject go2 = null;
                            if (PhysicsWorldManager.instance.gameObjectMap != null)
                            {
                                PhysicsWorldManager.instance.gameObjectMap.TryGetValue(rigidBody1, out go1);
                                PhysicsWorldManager.instance.gameObjectMap.TryGetValue(rigidBody2, out go2);
                            }
                            //if (FrameWriterSystem.Instance.RecordLog)
                            //    FrameWriterSystem.Instance.AddLog($"fullOverlaps.Remove {(go1 == null ? "go1 is null" : go1.name)}  {(go2 == null ? "go2 is null" : go2.name)}  {fullOverlaps.Count} ");
        ...
        public override void Detect()
        {
            ...
            if (FrameWriterSystem.Instance.RecordLog)
                FrameWriterSystem.Instance.AddLog($"{addCounter}  AddedObjectsBruteForceIsUsed{AddedObjectsBruteForceIsUsed} axis1 {axis1.Count} axis2{axis2.Count} axis3{axis3.Count}");

            addCounter = 0;
            for (int i = fullOverlaps.Count - 1; i >= 0; i--)
            {
                if (i < fullOverlaps.Count)
                {
            ...
            //    {
            //        OverlapPair key = fullOverlaps[i];
            //        RigidBody rigidBody1 = key.Entity1 as RigidBody;
            //        RigidBody rigidBody2 = key.Entity2 as RigidBody;
            ...
            //        }
            //    //    if (FrameWriterSystem.Instance.RecordLog)
            //    //        FrameWriterSystem.Instance.AddLog($"{(go1 == null?"go1 is null": go1.name)}  {(go2 == null?"go2 is null": go2.name)}  { fullOverlaps.Count} ");
            //    }
