...
namespace Framework.Lockstep
{
    ...
    public struct TSBBox
    {
        ...
        /// </summary>
        /// <param name="box">The other bounding box to check for intersection.</param>
        ...
        public bool Intersects(ref TSBBox box)
        {
            ...
            // If no separation found on any axis, boxes intersect
            return true;
        }

        /// <summary>
        /// Checks whether this bounding box intersects with another bounding box and returns the intersection volume.
        /// </summary>
        /// <param name="box">The other bounding box to check for intersection.</param>
        /// <param name="intersection">The resulting intersection box if boxes intersect.</param>
        /// <returns>True if the boxes intersect, false otherwise.</returns>
        public bool Intersects(ref TSBBox box, out TSBBox intersection)
        {
            if (Intersects(ref box))
            {
                // Calculate intersection box
                TSVector intersectionMin = TSVector.Max(this.min, box.min);
                TSVector intersectionMax = TSVector.Min(this.max, box.max);

                intersection = new TSBBox(intersectionMin, intersectionMax);
                return true;
            }

            intersection = new TSBBox();
            return false;
        }
