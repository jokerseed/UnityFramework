...
namespace Framework.Lockstep.Physics3D
{
    ...
    public class World : IWorld
    {
        ...
        public class WorldEvents
        {
            ...
            public void ClearAllEvents()
            {
                ...
                PostStep = null;
                AddedRigidBody = null;
                RemovedRigidBody = null;
                AddedConstraint = null;
                RemovedConstraint = null;
                AddedSoftBody = null;
                RemovedSoftBody = null;
                BodiesBeginCollide = null;
                BodiesStayCollide = null;
                BodiesEndCollide = null;
                TriggerBeginCollide = null;
                TriggerStayCollide = null;
                TriggerEndCollide = null;
                ContactCreated = null;
                DeactivatedBody = null;
                ActivatedBody = null;
            }
        ...
        {
            // 清空待处理队列
            removedArbiterQueue.Clear();
            addedArbiterQueue.Clear();
            removedArbiterStack.Clear();

            // 清空初始碰撞列表
            initialCollisions.Clear();
            initialTriggers.Clear();
            ...
            for (int index = 0, length = rigidBodies.Count; index < length; index++) {
                ...
                body.arbiters.Clear();
                body.arbitersTrigger.Clear(); // 清理触发器仲裁器
            ...
            arbiterCallback = null;
            integrateCallback = null;
            collisionDetectionHandler = null ;
        ...
        public void AddBody(RigidBody body)
        {
            ...
            GameObject go1 = null;
            if (PhysicsWorldManager.instance.gameObjectMap != null)
            {
                PhysicsWorldManager.instance.gameObjectMap.TryGetValue(body, out go1);
            }
            //if (FrameWriterSystem.Instance.RecordLog)
            //    FrameWriterSystem.Instance.AddLog($"AddBody 22222 {(go1 == null ? "go1 is null" : go1.name)} ");
   ...
   //         }
        ...
        {
            // 首先进行快速包围盒分离检测
            if (!arbiter.body1.boundingBox.Intersects(arbiter.body2.boundingBox))
            {
                // 立即移除所有接触点
            ...
            for (int i = arbiter.contactList.Count - 1; i >= 0; i--)
            {
                ...
                {
                    RemoveContactAndCheckArbiter(arbiter, i, c);
                    continue;
                }

                // 条件2：接触点滑动距离过大（原有的逻辑，但理解其真正目的）
                TSVector diff; TSVector.Subtract(ref c.p1, ref c.p2, out diff);
                FP penetration = TSVector.Dot(ref diff, ref c.normal); // 这是真正的穿透深度

                diff = diff - penetration * c.normal;
                FP slideDistanceSq = diff.sqrMagnitude;

                // 这个条件检测的是接触点是否滑动太远，不是检测分离！
                if (slideDistanceSq > contactSettings.breakThreshold * contactSettings.breakThreshold * 100)
                {
                    RemoveContactAndCheckArbiter(arbiter, i, c);
                    continue;
                }
                ...
                // 条件3：基于相对速度的分离检测（新增）
                TSVector relativeVel = arbiter.body1.linearVelocity - arbiter.body2.linearVelocity;
                FP separatingVelocity = TSVector.Dot(relativeVel, c.normal);

                if (separatingVelocity > FP.EN2 && c.penetration <= FP.Zero)
                {
                    RemoveContactAndCheckArbiter(arbiter, i, c);
                    continue;
                }
        ...
        private void RemoveContactAndCheckArbiter(Arbiter arbiter, int contactIndex, Contact contact)
        {
            Contact.Pool.GiveBack(contact);
            arbiter.contactList.RemoveAt(contactIndex);

            // 如果移除了最后一个接触点，立即标记仲裁器
        ...
        //private void UpdateArbiterContacts(Arbiter arbiter)
        //{
        //    bool shouldRemoveArbiter = true; // 假设应该移除仲裁器
        ...
        //        bool shouldRemoveContact = false;

        //        // 使用更保守的分离条件
        //        if (c.penetration < -contactSettings.breakThreshold * 2) // 需要更深的分离
        //        {
        //            shouldRemoveContact = true;
        //        }
        //        else
        //        {
        //            TSVector diff; TSVector.Subtract(ref c.p1, ref c.p2, out diff);
        ...
        //            // 使用更保守的阈值
        //            if (distance > contactSettings.breakThreshold * contactSettings.breakThreshold * 20)
        //            {
        //                shouldRemoveContact = true;
        //            }
        //        }
        ...
        //        else
        //        {
        //            // 如果有一个接触点不应该被移除，那么仲裁器也不应该被移除
        //            shouldRemoveArbiter = false;
        //        }
        //    }

        //    // 只有在确认所有接触点都应该被移除时才移除仲裁器
        //    if (shouldRemoveArbiter && arbiter.contactList.Count == 0)
        //    {
        //        lock (removedArbiterStack) { removedArbiterStack.Push(arbiter); }
        //    }
        //}
