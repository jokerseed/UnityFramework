...
namespace Framework.Lockstep.Physics2D
{
    /// <summary>
    /// The world class manages all physics entities, dynamic simulation,
    /// and asynchronous queries.
    /// </summary>
    public class World : Framework.Lockstep.IWorld
    {
        internal FP _invDt0;
        ...
        internal List<Joint2D> _jointAddList = new List<Joint2D>();
        internal List<Joint2D> _jointRemoveList = new List<Joint2D>();
        ...
        private void ProcessRemovedJoints()
        {
            ...
            {
                foreach (Joint2D joint in _jointRemoveList)
                {
        ...
        private void ProcessAddedJoints()
        {
            ...
            {
                foreach (Joint2D joint in _jointAddList)
                {
...
#else
            for (int index = 0, length = BodyList.Count; index < length; index++) {
                BodyList[index]._island = false;
            }
#endif
...
#else
            for (int index = 0, length = ContactManager.ContactList.Count; index < length; index++) {
                ContactManager.ContactList[index].IslandFlag = false;
            }
#endif
            for (int index = 0, length = JointList.Count; index < length; index++) {
                    JointList[index].IslandFlag = false;
            }
            ...
            for (; ; )
            {
                ...
                for (int i = 0; i < ContactManager.ContactList.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            alpha = Framework.FixedMath.TSMath.Min(alpha0 + (1.0f - alpha0) * beta, 1.0f);
                        }
                        else
                        {
                            alpha = 1.0f;
        ...
        public List<Joint2D> JointList { get; private set; }
        ...
        private void RemoveJoint(Joint2D joint, bool doCheck)
        {
            if (doCheck)
            {
        ...
        public void Step(FP dt)
        {
            ...
            for (int index = 0; index < BodyList.Count; index++) {
                List<IBodyConstraint> bConstraints = BodyList[index].bodyConstraints;
                for (int index2 = 0; index2 < bConstraints.Count; index2++) {
                    bConstraints[index2].PostStep();
                }
            }
        ...
        public List<Framework.Lockstep.IBody> Bodies() {
            List<Framework.Lockstep.IBody> bodies = new List<Framework.Lockstep.IBody>();
            for (int index = 0; index < BodyList.Count; index++) {
